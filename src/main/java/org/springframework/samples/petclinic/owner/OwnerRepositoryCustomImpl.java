...
public class OwnerRepositoryCustomImpl implements OwnerRepository {
	...
	public Collection<Owner> findByLastName(String lastName) {
		...
		// unsafe -- 検索機能を開発
		String sqlQuery = "SELECT DISTINCT owner FROM Owner owner left join fetch owner.pets WHERE owner.lastName LIKE '"
				+ lastName + "%'";
		TypedQuery<Owner> query = this.entityManager.createQuery(sqlQuery, Owner.class);
		// unsafe -- end

		// safe -- start
		// String sqlQuery = "SELECT DISTINCT owner FROM Owner owner left join fetch
		// owner.pets WHERE owner.lastName LIKE :lastName";
		// TypedQuery<Owner> query = this.entityManager.createQuery(sqlQuery,
		// Owner.class);
	...
	public void save(Owner owner) {
		...
		// If the object already exists, then we can't directly use the detached object in
		// persist.
		if (owner.getId() != null) {
